...
pyMAGIQ.vis.plotmap.plotmap(extent, latlist, lonlist, ratelist, fname)
...
for i in range(len(targetSiteID)):
    ...
    period, xmldata, Zapp, Zapp_err, Zphs, Zphs_err, _ = \
        pyMAGIQ.utils.iofiles.io.readXML(XMLpath, True)
...
y_val, rate_val, Zapplist, Zapp_errlist, Zphslist, Zphs_errlist = \
    pyMAGIQ.utils.MAGIQlib.MAGIQlib.xml2Z(xml_val,
                                          SiteID_val, nfreq,
                                          Rotate=False, freq_interp=freqsets)

X_val = pyMAGIQ.utils.MAGIQlib.MAGIQlib.getXfromZ(Zapplist, Zapp_errlist,
                                                  Zphslist, Zphs_errlist,
                                                  nfreq, SiteID_val)
...
pyMAGIQ.vis.plotz.plotz(alldata, targetSiteID, outputpath+'/AppRes_val.eps')
...
# Make XML list from xml files
xml_sens, SiteID_sens = \
    pyMAGIQ.utils.iofiles.io.get_XMLlists(nfreq, XMLsensdir, senslists, freqsets)

# read X (input data) and y (output data: rate) from XML files
y_sens, rate_sens, Zapplist, Zapp_errlist, Zphslist, Zphs_errlist = \
    pyMAGIQ.utils.MAGIQlib.MAGIQlib.xml2Z(xml_sens, SiteID_sens,
                                          nfreq, Rotate=True,
                                          freq_interp=freqsets)

X_sens = pyMAGIQ.utils.MAGIQlib.MAGIQlib.getXfromZ(Zapplist, Zapp_errlist,
                                                   Zphslist, Zphs_errlist,
                                                   nfreq, SiteID_sens)
