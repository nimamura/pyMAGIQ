...
def getXYdataset(X, y, nfreq, fdir, lists, freq_interp):
    ...
    for MTdirname in lists:
        ...
        period, xmldata, Zapp, Zapp_err, Zphs, Zphs_err, period_ori = \
            pyMAGIQ.utils.iofiles.io.readXML(XMLpath, True, freq_interp)
        # period, xmldata, Zapp, Zapp_err, Zphs, Zphs_err = utils.readXML(XMLpath,True,freq_interp)

        # Z = utils.readXML(XMLpath)
        # periodEDI, Z, ZERR, Zapp, Zapp_err, Zphs, Zphs_err = \
        #     gicio_edi.readEDIall(EDIpath)
        # periodEDI, Z, ZERR, Zapp, Zapp_err, Zphs, Zphs_err = \
        # gicio_edi.readEDIall(EDIpath, freq_interp)
        # nperiod = len(Zapp)

        # if maximum value periodEDI is smaller than 1000 sec,
        ...
        for j in range(nfreq):
            # if Z[j,0].real > 1.E+06:
            #     print MTdirname, 'over 1.E+07'

            # Normalize dataset and input it
            X[icount, j, 0] = (np.log10(Zapp[j, 1])+4)*0.01
            X[icount, j, 1] = (np.log10(Zapp[j, 2])+4)*0.01
            # X[icount, j,  2] = (np.log10( Zapp_err[j, 1] )+4)*0.01
            # X[icount, j,  3] = (np.log10( Zapp_err[j, 2] )+4)*0.01
            X[icount, j, 2] = (np.log10(Zapp_err[j, 1])+6)*0.01
            X[icount, j, 3] = (np.log10(Zapp_err[j, 2])+6)*0.01
            # Added 180.0 because phase could be goes under zero.
            # Also, we divided phase by 540 because it can go over 180.
            X[icount, j, 4] = (Zphs[j, 1]+180.0) / 540.0
            X[icount, j, 5] = (Zphs[j, 2]+180.0) / 540.0
            X[icount, j, 6] = Zphs_err[j, 1] / 540.0
            X[icount, j, 7] = Zphs_err[j, 2] / 540.0

            if j < nfreq-1:
                X[icount, j, 8] = np.abs((np.log10(Zapp[j, 1])-np.log10(Zapp[j+1, 1])))*0.01
                X[icount, j, 9] = np.abs((np.log10(Zapp[j, 2])-np.log10(Zapp[j+1, 2])))*0.01
                X[icount, j, 10] = \
                    np.abs((np.log10(Zapp_err[j, 1])-np.log10(Zapp_err[j+1, 1])))*0.01
                X[icount, j, 11] = \
                    np.abs((np.log10(Zapp_err[j, 2])-np.log10(Zapp_err[j+1, 2])))*0.01
                # X[icount, j, 8] = np.abs((np.log10(Zapp[j,1])-np.log10(Zapp[j+1,1])))*0.1
                # X[icount, j, 9] = np.abs((np.log10(Zapp[j,2])-np.log10(Zapp[j+1,2])))*0.1
                # X[icount, j, 10] = np.abs((np.log10(Zapp_err[j,1])-np.log10(Zapp_err[j+1,1])))*0.1
                # X[icount, j, 11] = np.abs((np.log10(Zapp_err[j,2])-np.log10(Zapp_err[j+1,2])))*0.1

                X[icount, j, 12] = np.abs((Zphs[j, 1]-Zphs[j+1, 1]) / 540.0)
                X[icount, j, 13] = np.abs((Zphs[j, 2]-Zphs[j+1, 2]) / 540.0)
                X[icount, j, 14] = np.abs((Zphs_err[j, 1]-Zphs_err[j+1, 1]) / 540.0)
                X[icount, j, 15] = np.abs((Zphs_err[j, 2]-Zphs_err[j+1, 2]) / 540.0)

            else:
                X[icount, j, 8] = np.abs((np.log10(Zapp[j-1, 1])-np.log10(Zapp[j, 1])))*0.01
                X[icount, j, 9] = np.abs((np.log10(Zapp[j-1, 2])-np.log10(Zapp[j, 2])))*0.01
                X[icount, j, 10] = \
                    np.abs((np.log10(Zapp_err[j-1, 1])-np.log10(Zapp_err[j, 1])))*0.01
                X[icount, j, 11] = \
                    np.abs((np.log10(Zapp_err[j-1, 2])-np.log10(Zapp_err[j, 2])))*0.01
                # X[icount, j, 8] = np.abs((np.log10(Zapp[j-1,1]) - np.log10(Zapp[j,1])))*0.1
                # X[icount, j, 9] = np.abs((np.log10(Zapp[j-1,2]) - np.log10(Zapp[j,2])))*0.1
                # X[icount, j, 10] = np.abs((np.log10(Zapp_err[j-1,1])-np.log10(Zapp_err[j,1])))*0.1
                # X[icount, j, 11] = np.abs((np.log10(Zapp_err[j-1,2])-np.log10(Zapp_err[j,2])))*0.1
                X[icount, j, 12] = np.abs((Zphs[j-1, 1]-Zphs[j, 1]) / 540.0)
                X[icount, j, 13] = np.abs((Zphs[j-1, 2]-Zphs[j, 2]) / 540.0)
                X[icount, j, 14] = np.abs((Zphs_err[j-1, 1]-Zphs_err[j, 1]) / 540.0)
                X[icount, j, 15] = np.abs((Zphs_err[j-1, 2]-Zphs_err[j, 2]) / 540.0)

            # print('Zapp   [%12.5e %12.5e %12.5e %12.5e] '\
            #     %(X[icount, j, 0], X[icount, j, 1], X[icount, j, 2],
            #       X[icount, j, 3]))
            # print('Zphs   [%12.5e %12.5e %12.5e %12.5e] '\
            #     %(X[icount, j, 8], X[icount, j, 9], X[icount, j, 10],
            #       X[icount, j, 11]))
            # print('Zapp_err [%12.5e %12.5e %12.5e %12.5e] '\
            #     %(X[icount, j, 4], X[icount, j, 5], X[icount, j, 6],
            #       X[icount, j, 7]))
            # print('Zphs_err [%12.5e %12.5e %12.5e %12.5e] '\
            #     %(X[icount, j, 12], X[icount, j, 13], X[icount, j, 14],
            #       X[icount, j, 15]))
...
def getX_unrated(X, nfreq, fdir, lists, freq_interp):
    ...
    for MTdirname in lists:
        ...
        try:
            period, xmldata, Zapp, Zapp_err, Zphs, Zphs_err, period_ori = \
                pyMAGIQ.utils.iofiles.io.readXML(XMLpath, True, freq_interp)
            # period, xmldata, Zapp, Zapp_err, Zphs, Zphs_err =\
            #     utils.readXML(XMLpath, True, freq_interp)
            # period, Z, ZERR, Zapp, Zapp_err, Zphs, Zphs_err = \
            #     utils.readEDIall(EDIpath, freq_interp)
        except ValueError:
        ...
        # if maximum value period is smaller than 100 sec, skip it because
        # interpolation won't work well
        if max(period_ori) < 10000.0:
            print('skip it! Max period is smaller than 10000 sec.', MTdirname)
            continue
        ...
        for j in range(nfreq):
            ...
            if j < nfreq-1:
                # X[icount, j, 8] = \
                #     np.abs((np.log10(Zapp[j, 1])-np.log10(Zapp[j+1, 1])))*0.1
                # X[icount, j, 9] = \
                #     np.abs((np.log10(Zapp[j, 2])-np.log10(Zapp[j+1, 2])))*0.1
                # X[icount, j, 10] = \
                #     np.abs((np.log10(Zapp_err[j, 1])-np.log10(Zapp_err[j+1, 1])))*0.1
                # X[icount, j, 11] = \
                #     np.abs((np.log10(Zapp_err[j, 2])-np.log10(Zapp_err[j+1, 2])))*0.1
                X[icount, j, 8] = \
                    np.abs((np.log10(Zapp[j, 1]) - np.log10(Zapp[j+1, 1]))) * 0.01
                X[icount, j, 9] = \
                    np.abs((np.log10(Zapp[j, 2]) - np.log10(Zapp[j+1, 2]))) * 0.01
                X[icount, j, 10] = \
                    np.abs((np.log10(Zapp_err[j, 1])-np.log10(Zapp_err[j+1, 1]))) * 0.01
                X[icount, j, 11] = \
                    np.abs((np.log10(Zapp_err[j, 2])-np.log10(Zapp_err[j+1, 2]))) * 0.01
                ...
                # X[icount, j, 8] = \
                #     np.abs((np.log10(Zapp[j-1, 1])-np.log10(Zapp[j, 1])))*0.1
                # X[icount, j, 9] = \
                #     np.abs((np.log10(Zapp[j-1, 2])-np.log10(Zapp[j, 2])))*0.1
                # X[icount, j, 10] = \
                #     np.abs((np.log10(Zapp_err[j-1, 1])-np.log10(Zapp_err[j, 1])))*0.1
                # X[icount, j, 11] = \
                #     np.abs((np.log10(Zapp_err[j-1, 2])-np.log10(Zapp_err[j, 2])))*0.1
                X[icount, j, 8] = \
                    np.abs((np.log10(Zapp[j-1, 1]) - np.log10(Zapp[j, 1]))) * 0.01
                X[icount, j, 9] = \
                    np.abs((np.log10(Zapp[j-1, 2]) - np.log10(Zapp[j, 2]))) * 0.01
                X[icount, j, 10] = \
                    np.abs((np.log10(Zapp_err[j-1, 1])-np.log10(Zapp_err[j, 1]))) * 0.01
                X[icount, j, 11] = \
                    np.abs((np.log10(Zapp_err[j-1, 2])-np.log10(Zapp_err[j, 2]))) * 0.01
    ...
    for icount in range(np.shape(Zapplist)[0]):
        ...
        for j in range(nfreq):
            ...
            if j < nfreq-1:
                ...
                X[icount, j, 12] = \
                    np.abs((Zphs[j, 1]-Zphs[j+1, 1]) / 540.0)
                X[icount, j, 13] = \
                    np.abs((Zphs[j, 2]-Zphs[j+1, 2]) / 540.0)
                X[icount, j, 14] = \
                    np.abs((Zphs_err[j, 1]-Zphs_err[j+1, 1]) / 540.0)
                X[icount, j, 15] = \
                    np.abs((Zphs_err[j, 2]-Zphs_err[j+1, 2]) / 540.0)
